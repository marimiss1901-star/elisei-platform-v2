...
assert.match(source,/Предварительно: текущая детализация WB ещё не пришла/,'estimated WB costs must be explicitly identified as provisional rather than confirmed values')
assert.match(source,/оценены по последнему подтверждённому периоду WB/,'historical fallback must disclose that it is based on confirmed WB history')
assert.match(source,/Подтверждённая детализация заменит оценки автоматически/,'provisional estimates must promise automatic replacement by confirmed finance')
...
console.log('ELISEI P&L and dashboard missing-data regressions passed')
