...
assert.deepEqual(LIVE_SYNC_STAGES,['orders','sales','stocks','sellerStocks'],
  'seller-day recurring lane must contain only orders, sales and FBO/FBS stocks')
assert.deepEqual(DAILY_READY_OPERATIONAL_RECOVERY_STAGES,['orders','sales'],
  'advertising must not be recovered as a daytime operational stream')
...
console.log('Seller-day/night load policy regression passed')
