...
assert.match(source,/const logisticsSource = financeHasRows \? 'wb_api' : Number\(settings\.logisticsPerSale \|\| 0\) > 0 \? 'manual' : 'not_loaded'/,
  'missing logistics must be marked as not loaded instead of confirmed zero')
assert.match(source,/logistics: logisticsSource === 'not_loaded' \? null : Math\.round\(logistics\)/,
  'product logistics must expose null when WB detail and fallback are unavailable')
assert.match(source,/acquiring: acquiringSource === 'not_loaded' \? null : Math\.round\(acquiring\)/,
  'product acquiring must expose null when it is unavailable')
...
console.log('ELISEI 5.19.3 product P&L cost truthfulness regression: OK')
