...
assert.ok(route.includes("version: '5.11.1'"),'El Core status must report 5.10.1')

const server = fs.readFileSync(new URL('../src/server.js',import.meta.url),'utf8')
assert.ok(server.includes("version: '2.23.1'"),'backend health must report 2.22.1')
assert.ok(server.includes('ELISEI/2.23.1'),'WB User-Agent must use backend release version')
