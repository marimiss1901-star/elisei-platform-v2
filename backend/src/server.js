...
import {
  ...
  DAILY_READY_QUARTER_STAGES, yesterdayDateKey, shiftIsoDate, nightlyQuarterTaxPeriod,
  dailyReadySlot, dailyHeavyStagePlan, dailyOperationalStageCovered,
...
async function loadGoodsReturns(token, { deadlineAt = 0, period: requestedPeriod = null } = {}) {
  const period = requestedPeriod && (requestedPeriod.dateFrom || requestedPeriod.from) && (requestedPeriod.dateTo || requestedPeriod.to)
    ? {
        dateFrom:dateKey(requestedPeriod.dateFrom || requestedPeriod.from),
        dateTo:dateKey(requestedPeriod.dateTo || requestedPeriod.to),
        days:Number(requestedPeriod.days || 30),
      }
    : reportPeriod(30)
...
async function runSyncStage({ connection, tokens, data, stage, deadlineAt }) {
  ...
  try {
    ...
    if (stage === 'products') {
      ...
      const loaded=await loadGoodsReturns(selected.token,{deadlineAt,period:state?.metadata?.period || null})
      value=loaded.value; meta=loaded.validation; snapshot=loaded
...
function elCompactProduct(item = {}) {
  return {
    ...
    acquiring: item.acquiring ?? null,
    penalties: item.penalties ?? null,
    deductions: item.deductions ?? null,
    additionalPayment: item.additionalPayment ?? null,
    tax: item.tax ?? null,
    fixedExpenses: item.fixedExpenses ?? null,
    advertising: item.advertising ?? item.adSpend ?? null,
    adRevenue: item.adRevenue ?? null,
    adOrders: item.adOrders ?? null,
    adCampaignIds: Array.isArray(item.adCampaignIds) ? item.adCampaignIds : [],
    financeSource: item.financeSource || null,
    acquiringSource: item.acquiringSource || null,
    storageSource: item.storageSource || null,
...
async function buildElModuleData({ req, identity, period, module, focus }) {
  ...
  if (module === 'advertising') {
    const selectedCampaigns = Array.isArray(core.advertising?.campaigns) ? core.advertising.campaigns : []
    const rawAdvertising = canonical.data?.advertising && typeof canonical.data.advertising === 'object' ? canonical.data.advertising : {}
    const rawCampaigns = Array.isArray(rawAdvertising.campaigns) ? rawAdvertising.campaigns : []
    const rawStatsCampaigns = rawCampaigns.filter(item =>
      item?.statsStatus === 'loaded'
    ...
    const useSnapshotFallback = Boolean(
      filtered.range
      && !core.advertising?.statsAvailable
      && rawStatsCampaigns.length
    )
    const advertisingPayload = useSnapshotFallback ? rawAdvertising : (core.advertising || {})
    return {
      ...base,
      summary: {
        spend: useSnapshotFallback ? (advertisingPayload?.totals?.spend ?? core.summary.advertising) : core.summary.advertising,
        source: core.summary.advertisingSource,
        operatingProfit: core.summary.operatingProfit,
        margin: core.summary.margin,
      },
      advertising: {
        totals: advertisingPayload?.totals || {},
        period: advertisingPayload?.period || core.advertising?.period || null,
        statsAvailable: Boolean(!useSnapshotFallback && core.advertising?.statsAvailable),
        snapshotFallback: useSnapshotFallback,
        campaigns: (useSnapshotFallback ? rawStatsCampaigns : selectedCampaigns).slice(0, 80),
        productRows: (advertisingPayload?.productRows || core.advertising?.productRows || []).slice(0, 100),
      },
      warning: useSnapshotFallback
        ? 'Точного рекламного среза за выбранный период пока нет; использован последний сохранённый снимок кампаний WB.'
        : null,
  ...
  if (module === 'finance') {
    return {
      ...
      productPnlRows: products
        .filter(item => Number(item.revenue || item.salesCount || item.advertising || item.expenses || 0) !== 0 || item.profit != null)
        .sort((a,b) => Math.abs(Number(b.profit ?? b.revenue ?? 0))-Math.abs(Number(a.profit ?? a.revenue ?? 0)))
        .slice(0, 150)
        .map(elCompactProduct),
...
async function scheduleDailyReadyStages() {
  ...
  const taxQuarter=nightlyQuarterTaxPeriod(new Date(now),dailyReadyTimezone)
  const taxQuarterRange={from:taxQuarter.dateFrom,to:taxQuarter.dateTo,days:taxQuarter.days}
  for(const row of rows){
    try{
      ...
      if(financeStale && !financeHardBlocked.has(financeStatus) && !financeContinuationActive){
        ...
        await updateSyncState(row.id,'finance',{
          ...
          metadata:{
            ...
            nightlyReady:true,nightlyReadyVersion:3,financeFreshnessRepair:true,quarterlyTaxReady:true,taxQuarter,
      ...
      for(const [heavyIndex,stage] of heavyPlan.entries()){
        ...
        const quarterPeriod=DAILY_READY_QUARTER_STAGES.includes(stage)
          ? syncPeriodForStage(stage,taxQuarterRange)
          : null
        const quarterMetadata=quarterPeriod ? {
          ...initialPeriodStageMetadata(stage,quarterPeriod),
        ...
        const metadata=pagedFinance
          ? {
              ...
              nightlyReady:true,nightlyReadyVersion:3,quarterlyTaxReady:Boolean(quarterPeriod),taxQuarter,
              period:quarterPeriod || reportPeriod(30),syncId:crypto.randomUUID(),rrdId:'0',pageNumber:0,persistedCount:0,
            }
          : quarterMetadata
            ? {
                ...quarterMetadata,
                trigger:'nightly_ready',dailyReadySlot:slot,dailyReadyDate:targetDate,
                nightlyReady:true,nightlyReadyVersion:3,
              }
          : {
              ...(current?.metadata || {}),
              trigger:'nightly_ready',dailyReadySlot:slot,dailyReadyDate:targetDate,
