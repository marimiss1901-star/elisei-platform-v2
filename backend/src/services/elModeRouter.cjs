...
const BUSINESS_RE = /(wildberries|вайлдбер|\bwb\b|кабинет|продаж|заказ|выручк|оборот|реклам|кампан|дрр|roas|romi|cpc|ctr|остат|склад|дефицит|оборачиваем|прибыл|марж|p&l|pnl|себестоим|комисси|логистик|расход|товар|артикул|nmid|карточк|размер|штрихкод|возврат|отказ|выкуп|fbs|фбс|fbo|фбо|отзыв|рейтинг|цен|скидк|акци|сезон|закуп|дозаказ|синхрон|импорт|качество данных)/i;
