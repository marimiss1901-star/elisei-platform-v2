...
import {
  PERIOD_MODES,
  getPeriodLabel,
  installPeriodFetchInterceptor,
  setPeriod,
  shiftPeriod,
  useGlobalPeriod,
} from '../lib/periodStore.js';
...
function nativePickerType(mode) {
  if (mode === PERIOD_MODES.MONTH) return 'month';
  return 'date';
}

function anchorValue(period) {
  if (period.mode === PERIOD_MODES.MONTH) return period.anchor.slice(0, 7);
...
export default function GlobalPeriodBar({ compact = false }) {
  ...
  useEffect(() => installPeriodFetchInterceptor(), []);
  useEffect(() => {
  ...
  const changeAnchor = (value) => {
    const normalized = period.mode === PERIOD_MODES.MONTH ? `${value}-01` : value;
    setPeriod({ anchor: normalized });
  };
  ...
  return (
    <section className={`elisei-period-bar ${compact ? 'is-compact' : ''}`} aria-label="Выбор периода аналитики">
      <div className="elisei-period-modes" role="tablist" aria-label="Тип периода">
        {modeOptions.map(([mode, text]) => (
          <button
            key={mode}
            type="button"
            className={period.mode === mode ? 'is-active' : ''}
            onClick={() => {
              setPeriod({ mode, anchor: period.anchor });
              setOpen(mode === PERIOD_MODES.CUSTOM);
            }}
          >
            {text}
          </button>
      ...
      <div className="elisei-period-controls">
        ...
        {period.mode === PERIOD_MODES.CUSTOM ? (
          ...
          <input
            className="period-native-input"
            type={nativePickerType(period.mode)}
            value={anchorValue(period)}
            onChange={(event) => changeAnchor(event.target.value)}
            aria-label="Дата периода"
          />
        )}
      ...
      <label className="elisei-period-compare">
        <input
          ...
          onChange={(event) => setPeriod({ compareEnabled: event.target.checked })}
        />
        <span>Сравнить</span>
      </label>
