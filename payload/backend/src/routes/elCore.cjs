...
function createRouter(express) {
  ...
  router.get('/status', asyncRoute(async (req, res) => {
    ...
    res.json({
      ok: true, version: '5.3.20', name: 'El Whole Business Brain', configured: Boolean(process.env.OPENAI_API_KEY),
  ...
  router.get('/capabilities', (req, res) => res.json({ ok: true, version: '5.3.20', modules: publicCapabilities(), writeActions: false }));
