...
## ELISEI 5.3.3
Фоновая очередь WB запускается не только таймером backend, но и heartbeat-запросами открытого интерфейса. Это устраняет ситуацию, когда next_allowed_at уже прошёл, а этап остаётся в статусе «Пауза Wildberries».
