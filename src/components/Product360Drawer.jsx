...
export default function Product360Drawer({ product, data, loading, error, period, onClose }) {
  ...
  return <div className="sku360-backdrop" onClick={onClose}>
    <aside className="sku360-drawer" onClick={event=>event.stopPropagation()}>
      ...
      <div className="sku360-grid two">
        <Section icon={Megaphone} eyebrow="Реклама" title="Эффективность кампаний" side={<span className="sku360-side-note">привязка по nmID</span>}>
        ...
        <Section icon={Search} eyebrow="Поисковая видимость" title="Что приводит покупателей" side={streams.searchQueries?.periodExact===false?<span className="sku360-warning-chip">снимок другого периода</span>:null}>
          {searchRows.length ? <><div className="sku360-note">Только реальные поисковые запросы, которые WB связал с nmID товара. Подменные SKU/промо-показы исключены.</div><div className="sku360-search-table"><div className="head"><span>Запрос</span><span>Позиция</span><span>Заказы</span><span>Выручка</span></div>{searchRows.slice(0,12).map(row=><div key={row.id}><span><strong>{row.phrase || 'Запрос'}</strong><small>{row.frequency!=null?`частотность ${fmtNum(row.frequency)}`:''}</small></span><b>{row.avgPosition==null?'—':fmtNum(row.avgPosition)}</b><b>{fmtNum(row.orders)}</b><b>{fmtMoney(row.revenue)}</b></div>)}</div></> : <div className="sku360-empty">{streams.searchQueries?.searchBindingVerified===false ? 'Перепроверяем принадлежность поисковых фраз по nmID. Старые непроверенные строки скрыты.' : readiness.search === 'ready' ? 'Подтверждено: поисковых фраз по этому SKU в выбранной выборке нет.' : readiness.search === 'partial' ? 'Поиск загружен частично; фразы по SKU ещё могут появиться.' : 'Поисковые данные по SKU ещё ожидаются.'}</div>}
        </Section>
      </div>
