...
export default function LoginPage({ onNavigate, onLogin }) {
  ...
  const [recovery, setRecovery] = useState({ phone:'', code:'', password:'', confirm:'' })
  ...
  const openRecovery = () => {
    clearNotices()
    setRecovery({ phone:'', code:'', password:'', confirm:'' })
    setSmsSent(false)
    setMode('request')
  }
  const backToLogin = () => {
  ...
  const requestResetSms = async (e) => {
    e.preventDefault(); clearNotices(); setLoading(true)
    try {
      const result = await authApi.requestPasswordResetSms({ phone:recovery.phone })
      setSmsSent(true)
      setMode('confirm')
      setMessage(result.message || 'Код отправлен по SMS. Он действует 5 минут.')
    } catch (err) { setError(err.message) } finally { setLoading(false) }
  }

  const confirmResetSms = async (e) => {
    e.preventDefault(); clearNotices()
    if (recovery.code.length !== 6) return setError('Введите шестизначный код из SMS.')
  ...
  const resendSms = async () => {
    ...
    try {
      const result = await authApi.requestPasswordResetSms({ phone:recovery.phone })
      setSmsSent(true)
      setMessage(result.message || 'Новый код отправлен по SMS.')
    } catch (err) { setError(err.message) } finally { setLoading(false) }
  }
  ...
  return <div className="auth-page"><button className="auth-back" onClick={()=>onNavigate('/')}><ArrowLeft size={17}/> На главную</button><section className="auth-card glass-panel"><div className="auth-brand"><span>E</span><strong>ELISEI</strong></div><div className="auth-el"><ElMascot compact/></div>
    ...
    {mode === 'request' && <><h1>Восстановить доступ</h1><p>Введите телефон, привязанный к аккаунту. Если аккаунт был создан до появления подтверждения телефона, используйте номер восстановления, назначенный владельцу ELISEI.</p><form onSubmit={requestResetSms}><label><span>Телефон восстановления</span><div><Phone size={17}/><input type="tel" value={recovery.phone} onChange={e=>setRecovery({...recovery,phone:e.target.value})} placeholder="+7 999 123-45-67" autoComplete="tel" required/></div></label>{error && <div className="auth-error">{error}</div>}{message && <div style={successStyle}>{message}</div>}<button className="primary-btn auth-submit" disabled={loading || !recovery.phone.trim()}>{loading ? 'Отправляем код…' : <>Получить SMS-код <KeyRound size={17}/></>}</button></form><small><button onClick={backToLogin}>Вернуться ко входу</button></small></>}

    {mode === 'confirm' && <><h1>Новый пароль</h1><p>Код отправлен на номер <b>{recovery.phone}</b>. Введите его и задайте новый пароль.</p><form onSubmit={confirmResetSms}><label><span>Код из SMS</span><div><ShieldCheck size={17}/><input inputMode="numeric" autoComplete="one-time-code" value={recovery.code} onChange={e=>setRecovery({...recovery,code:e.target.value.replace(/\D/g,'').slice(0,6)})} placeholder="000000" maxLength={6} required/></div></label><label><span>Новый пароль</span><div><LockKeyhole size={17}/><input type="password" value={recovery.password} onChange={e=>setRecovery({...recovery,password:e.target.value})} placeholder="Минимум 8 символов" required minLength={8} autoComplete="new-password"/></div></label><label><span>Повторите пароль</span><div><LockKeyhole size={17}/><input type="password" value={recovery.confirm} onChange={e=>setRecovery({...recovery,confirm:e.target.value})} placeholder="Повторите пароль" required minLength={8} autoComplete="new-password"/></div></label>{error && <div className="auth-error">{error}</div>}{message && <div style={successStyle}>{message}</div>}<button className="primary-btn auth-submit" disabled={loading || recovery.code.length !== 6}>{loading ? 'Сохраняем…' : <>Сохранить новый пароль <ArrowRight size={17}/></>}</button><button className="secondary-btn" type="button" onClick={resendSms} disabled={loading || !smsSent}><Phone size={17}/> Отправить код ещё раз</button></form><small><button onClick={backToLogin}>Вернуться ко входу</button></small></>}
  </section></div>
}
