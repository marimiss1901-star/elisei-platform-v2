...
export default function LoginPage({ onNavigate, onLogin }) {
  ...
  return <div className="auth-page"><button className="auth-back" onClick={()=>onNavigate('/')}><ArrowLeft size={17}/> На главную</button><section className="auth-card glass-panel"><div className="auth-brand"><span>E</span><strong>ELISEI</strong></div><div className="auth-el"><ElMascot compact/></div>
    ...
    {mode === 'request' && <><h1>Восстановить доступ</h1><p>Введите номер телефона, который был подтверждён при регистрации. ELISEI отправит шестизначный код по SMS.</p><form onSubmit={requestResetSms}><label><span>Телефон аккаунта</span><div><Phone size={17}/><input type="tel" value={recovery.phone} onChange={e=>setRecovery({...recovery,phone:e.target.value})} placeholder="+7 999 123-45-67" autoComplete="tel" required/></div></label>{error && <div className="auth-error">{error}</div>}{message && <div style={successStyle}>{message}</div>}<button className="primary-btn auth-submit" disabled={loading || !recovery.phone.trim()}>{loading ? 'Отправляем код…' : <>Получить SMS-код <KeyRound size={17}/></>}</button></form><small><button onClick={backToLogin}>Вернуться ко входу</button></small></>}
