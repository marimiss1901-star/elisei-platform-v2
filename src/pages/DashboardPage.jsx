...
const lastClosedTaxDate = () => addDays(isoLocalDate(new Date()),-1)
...
const quarterPeriodFor = (year, quarter) => {
  ...
  const closedDate = lastClosedTaxDate()
  ...
  const to = from > closedDate ? quarterEnd : earlierIsoDate(quarterEnd,closedDate)
  return { preset:`tax-q${numericQuarter}`,from,to,year:numericYear,quarter:numericQuarter,closedDate }
}
...
const taxDeadlinesFor = period => {
  ...
  const secondMonth = String(quarter*3+2).padStart(2,'0')
  const thirdMonth = String(quarter*3+3).padStart(2,'0')
  return {
    usn:`Аванс УСН: до 28.${nextMonth}.${year}`,
    vat:`НДС за квартал: 28.${nextMonth}.${year}, 28.${secondMonth}.${year}, 28.${thirdMonth}.${year} по 1/3`,
  }
}
...
export default function DashboardPage({ onNavigate, onLogout, user, onUserUpdate }) {
  ...
  const renderTax = () => {
    ...
    const quarterComplete = analyticsPeriod.to >= selectedQuarterPeriod.to
    const taxTemplateText = [
      'Предварительный расчет налогового резерва по кабинету WB',
      `Период: ${formatDate(analyticsPeriod.from)} — ${formatDate(analyticsPeriod.to)}.`,
      `Налоговый режим для управленческого расчета: УСН «доходы» 6% + НДС 5%.`,
      `База для УСН и НДС: ${formatMoney(taxBase)}.`,
      `УСН «доходы»: ${formatMoney(taxBase)} × ${usnRate}% = ${formatMoney(usnTax)}. Срок: ${deadlines.usn}.`,
      `НДС 5%: ${formatMoney(taxBase)} × ${vatRate}% = ${formatMoney(vatTax)}. ${deadlines.vat}.`,
      `НДС к оплате по 1/3: ${formatMoney(vatPart)} за каждый платеж.`,
      `Итого налоговый резерв: ${formatMoney(totalTax)}.`,
      'Комментарий: для УСН «доходы» выручка маркетплейса учитывается до удержания комиссии WB, логистики и прочих расходов. Расчет предварительный, итоговую декларацию и право применения ставки НДС 5% нужно сверить с бухгалтером.',
    ].join('\n')
    const copyTaxTemplate = async () => {
      try {
        await navigator.clipboard.writeText(taxTemplateText)
        notify('Шаблон для налоговой скопирован.')
      } catch {
        notify('Не удалось скопировать автоматически. Текст шаблона можно выделить вручную.')
      }
    }
    ...
    return <section className="app-page glass-panel tax-page">
      <div className="analytics-title-row">
        <div className="page-title"><span>Налоговая модель</span><h1>Налоговая</h1><p>Предварительный расчёт по УСН «доходы» и НДС 5% на основе сохранённой выручки WB за закрытые дни квартала.</p></div>
      ...
      <div className="notice info tax-period-note"><CalendarDays size={20}/><div><strong>Ночной квартальный сбор включён</strong><p>ELISEI ночью догружает период {formatDate(selectedQuarterPeriod.from)} — {formatDate(selectedQuarterPeriod.to)}. Сегодняшний день не входит в налоговую базу, пока он не закрыт WB.</p></div><b>{quarterComplete?'Период выбран':'Выберите квартал'}</b></div>
      ...
      <div className="tax-template-card settings-card">
        <div className="section-title-row"><div><span>Шаблон</span><h2>Текст для налоговой / бухгалтера</h2></div><button className="secondary-btn" onClick={copyTaxTemplate}><FileText size={16}/> Скопировать</button></div>
        <pre>{taxTemplateText}</pre>
      </div>
