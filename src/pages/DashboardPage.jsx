...
export default function DashboardPage({ onNavigate, onLogout }) {
  ...
  useEffect(() => {
    ...
    wbApi.status(connectionId).then(status => {
      ...
      return Promise.all([wbApi.dashboard(connectionId), wbApi.products(connectionId)])
    }).then(([dashboard, productResult]) => {
  ...
  const productRows = liveProducts.length ? liveProducts.map(p => [String(p.vendorCode || p.nmID || '—'), p.title || 'Товар', '—', '—', 'Загружен']) : products
  ...
  const disconnect = async () => {
    ...
    setDashboardData(null); setLiveProducts([]); setTokenDraft('')
