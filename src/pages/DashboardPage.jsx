...
export default function DashboardPage({ onNavigate, onLogout, user }) {
  ...
  const notify = (text, duration = 3600) => { setToast(text); window.clearTimeout(window.__eliseiToast); window.__eliseiToast=setTimeout(()=>setToast(''), duration) }
  ...
  const syncConnection = async (connectionId = connection.connectionId) => {
    ...
    try {
      ...
      if (result.warnings?.length) {
        notify(`Синхронизация завершена частично. ${result.warnings[0]}`, 9000)
      } else {
  ...
  const saveConnection = async (e) => {
    ...
    try {
      ...
      setConnection(next)
      setTokenDraft('')
      notify('Wildberries подключён. Теперь запустите синхронизацию один раз.', 6000)
      setActive('Синхронизации')
    } catch (error) { notify(error.message) }
