...
export default function DashboardPage({ onNavigate, onLogout, user }) {
  ...
  const renderStocks = () => {
    ...
    const mappingNeedsRefresh = stockAvailable && Number(stockMeta?.totalQuantity || 0) > 0 && !stockDetailsAvailable && Boolean(stockMeta?.needsIdentifierRefresh || stockMeta?.legacySnapshot)
    ...
    const repairStockMapping = () => syncConnection(connection.connectionId, ['products', 'stocks'])
    ...
    return <section className="app-page glass-panel"><div className="page-title"><span>Управление запасами</span><h1>Остатки</h1><p>Дни запаса, дефицит, излишки, замороженные деньги и план пополнения.</p></div>{requireConnection(<>
      ...
      {mappingNeedsRefresh && <div className="notice warning"><AlertTriangle size={20}/><div><strong>Остатки получены, но не привязаны к артикулам</strong><p>WB вернул {formatNumber(stockMeta?.totalQuantity || 0)} шт., однако сохранённый каталог не содержит идентификаторов размеров chrtID. ELISEI обновит карточки с размерами, затем заново получит снимок остатков и распределит его по артикулам.</p></div><button disabled={syncing || stockState?.status === 'pending'} onClick={repairStockMapping}>Обновить товары и остатки</button></div>}
      ...
      {stockAvailable && coreData?.stockDetails?.length > 0 && <><div className="section-title-row"><div><span>Детализация</span><h2>По артикулам, размерам и складам</h2></div><small>{formatNumber(coreData.stockDetails.length)} строк</small></div><div className="data-table stock-detail-table"><div className="data-row head stock-detail-row"><span>Товар</span><span>Размер</span><span>chrtID / штрихкод</span><span>Склад</span><span>Остаток</span></div>{coreData.stockDetails.slice(0,500).map(row => <div className="data-row stock-detail-row" key={row.key}><span><strong>{row.title}</strong><small>{row.vendorCode || `nmID ${row.nmID || '—'}`}</small></span><span>{row.techSize || '—'}</span><span>{row.chrtId || row.barcode || '—'}{row.chrtId && row.barcode ? <small>{row.barcode}</small> : null}</span><span>{row.warehouseName}</span><span>{formatNumber(row.quantity)}</span></div>)}</div></>}
      {unmatchedRows.length > 0 && <><div className="section-title-row"><div><span>Диагностика</span><h2>Не сопоставленные строки WB</h2></div><small>{formatNumber(unmatchedRows.length)} строк</small></div><div className="data-table stock-detail-table"><div className="data-row head stock-detail-row"><span>Идентификатор</span><span>Размер</span><span>chrtID / штрихкод</span><span>Склад</span><span>Остаток</span></div>{unmatchedRows.slice(0,200).map(row => <div className="data-row stock-detail-row" key={row.key}><span><strong>{row.vendorCode || `nmID ${row.nmID || 'не передан'}`}</strong><small>Строка пока не привязана к карточке</small></span><span>{row.techSize || '—'}</span><span>{row.chrtId || row.barcode || '—'}</span><span>{row.warehouseName}</span><span>{formatNumber(row.quantity)}</span></div>)}</div></>}
    </>)}</section>
  }
