(function () {
  ...
  async function postChat(payload){
    ...
    for(var i=0;i<urls.length;i++){
      try{
        var authToken=localStorage.getItem('elisei_auth_token')||''; var response=await fetch(urls[i],{method:'POST',headers:{'Content-Type':'application/json','X-Cabinet-Id':payload.cabinetId,'X-Cabinet-Name':encodeURIComponent(payload.cabinetName),...(authToken?{Authorization:'Bearer '+authToken}:{})},body:JSON.stringify(payload)});
